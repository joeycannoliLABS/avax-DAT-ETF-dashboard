...
const DATS = [
  ...
  },
  {
    ...
    highlights: ["Foundation raising $1B for two treasury cos","AVAX purchased at discounted price from Foundation","Initial $460M AUM targeting $1B","0.77x mNAV pricing noted","To trade on NASDAQ"],
    color: "#F59E0B", logo: "🏛️"
  }
];
...
const ETFS = [
  {
    ...
    highlights: ["First & only U.S.-listed AVAX spot ETP","Staking via Coinbase (4% service fee)","Anchorage Digital Bank as primary custodian","~5.57% gross staking yield reported","State Street as cash custodian & admin"],
    color: "#0066FF", logo: "📈"
  },
  {
    id: "grayscale-avax",
    ...
    highlights: ["Launched Aug 2024 as private trust","S-1 filed Aug 2025 for ETF conversion","Up to 70-85% of AVAX may be staked","Coinbase Custody + BNY Mellon admin","Ticker to change to GAVX on NASDAQ"],
    color: "#6B7280", logo: "🔷"
  },
  {
    id: "bitwise-bava",
...
function AvaxLogo() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <svg viewBox="0 0 40 40" style={{ height: 40, width: 40 }} xmlns="http://www.w3.org/2000/svg">
        <circle cx="20" cy="20" r="20" fill="#E84142"/>
        <text x="20" y="27" textAnchor="middle" fill="white" fontSize="22" fontWeight="bold">A</text>
      </svg>
      <span style={{ fontSize: 26, fontWeight: 700, color: "#E84142", fontFamily: "'Inter', sans-serif" }}>avalanche</span>
    </div>
  );
}
...
function StatusBadge({ status }) {
  ...
  const isPending = s.includes("pending") || s.includes("filed") || s.includes("awaiting");
  const bg = isLive
  ...
  return <span style={{ fontSize: 11, fontWeight: 500, padding: "2px 8px", borderRadius: 9999, border: "1px solid", whiteSpace: "nowrap", ...bg }}>{status}</span>;
}

function SectionHeader({ icon, title, subtitle, accent }) {
  return (
    <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
      <div style={{ width: 40, height: 40, borderRadius: 10, background: `linear-gradient(135deg, ${accent}33, ${accent}11)`, border: `1px solid ${accent}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{icon}</div>
      <div>
...
function EntityCard({ e, price, isOpen, onToggle }) {
  ...
  return (
    <div onClick={onToggle} style={{
      ...
      transition: "all 0.2s", boxShadow: isOpen ? `0 0 24px ${e.color}18` : "none"
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 8 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: e.color, fontWeight: 600 }}>{e.ticker} • {e.exchange}</div>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
        <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: "10px 12px" }}>
          ...
          {holdingsVal && <div style={{ fontSize: 11, color: "#94a3b8" }}>≈ {fmt(holdingsVal)}</div>}
          {!e.avaxHoldings && e.aum && <div style={{ fontSize: 11, color: "#94a3b8" }}>AUM: {fmt(e.aum)}</div>}
        </div>
        <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: "10px 12px" }}>
          ...
          {e.feeWaiver && <div style={{ fontSize: 10, color: "#4ade80", lineHeight: 1.3 }}>{e.feeWaiver}</div>}
          {e.stakingPct && !e.sponsorFee && <div style={{ fontSize: 10, color: "#4ade80" }}>of holdings staked</div>}
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const fetchPrice = useCallback(async () => {
    try {
      const r = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=avalanche-2&vs_currencies=usd&include_24hr_change=true&include_market_cap=true&include_24hr_vol=true");
      if (!r.ok) throw new Error("API error");
      const d = await r.json();
      const a = d["avalanche-2"];
      setPrice(a.usd); setPriceChange(a.usd_24h_change); setMktCap(a.usd_market_cap); setVol24(a.usd_24h_vol);
      setLastUpdate(new Date()); setErr(null);
    } catch {
      if (!price) { setPrice(8.86); setPriceChange(-1.5); setMktCap(3827000000); setVol24(261000000); setLastUpdate(new Date()); setErr("Using cached data"); }
    } finally { setLoading(false); }
  }, [price]);

  useEffect(() => { fetchPrice(); const i = setInterval(fetchPrice, 60000); return () => clearInterval(i); }, [fetchPrice]);

  const totalDATAvax = DATS.reduce((s, e) => s + (e.avaxHoldings || 0), 0);
  const totalETFAum = ETFS.reduce((s, e) => s + (e.aum || 0), 0);

  return (
    <div style={{ background: "linear-gradient(135deg, #0f0f1a 0%, #1a1025 50%, #0f1a2e 100%)", minHeight: "100vh", color: "#e2e8f0", fontFamily: "'Inter', -apple-system, system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          {lastUpdate && <p style={{ color: "#64748b", fontSize: 11, marginTop: 4 }}>Updated: {lastUpdate.toLocaleTimeString()} {err && <span style={{ color: "#fbbf24" }}>• {err}</span>}</p>}
        </div>

        {/* Price Banner */}
        <div style={{ background: "rgba(232,65,66,0.08)", border: "1px solid rgba(232,65,66,0.2)", borderRadius: 16, padding: "18px 24px", marginBottom: 28, display: "flex", flexWrap: "wrap", gap: 20, alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 1, marginBottom: 2 }}>AVAX / USD</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <span style={{ fontSize: 34, fontWeight: 800, color: "#fff" }}>{loading ? "..." : `$${price?.toFixed(2)}`}</span>
              {priceChange != null && <span style={{ fontSize: 15, fontWeight: 600, color: priceChange >= 0 ? "#4ade80" : "#f87171" }}>{priceChange >= 0 ? "▲" : "▼"} {Math.abs(priceChange).toFixed(2)}%</span>}
            </div>
          </div>
          <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
            <div><div style={{ fontSize: 10, color: "#94a3b8" }}>Market Cap</div><div style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>{fmt(mktCap)}</div></div>
            <div><div style={{ fontSize: 10, color: "#94a3b8" }}>24h Volume</div><div style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>{fmt(vol24)}</div></div>
            <div><div style={{ fontSize: 10, color: "#94a3b8" }}>DAT AVAX Holdings</div><div style={{ fontSize: 17, fontWeight: 700, color: "#E84142" }}>{fmtAvax(totalDATAvax)}</div>{price && <div style={{ fontSize: 10, color: "#94a3b8" }}>≈ {fmt(totalDATAvax * price)}</div>}</div>
            <div><div style={{ fontSize: 10, color: "#94a3b8" }}>ETF Total AUM</div><div style={{ fontSize: 17, fontWeight: 700, color: "#0066FF" }}>{fmt(totalETFAum)}</div></div>
          </div>
        </div>

        {/* DAT Section */}
        <SectionHeader icon="🏦" title="Digital Asset Treasuries" subtitle="Publicly traded companies accumulating AVAX as a treasury asset" accent="#E84142" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 16, marginBottom: 36 }}>
          {DATS.map(e => <EntityCard key={e.id} e={e} price={price} isOpen={selected === e.id} onToggle={() => setSelected(selected === e.id ? null : e.id)} />)}
        </div>

        <div style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent)", marginBottom: 36 }} />

        {/* ETF Section */}
        <SectionHeader icon="📊" title="Exchange-Traded Funds" subtitle="Spot AVAX ETFs offering regulated exposure and staking rewards" accent="#0066FF" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 16, marginBottom: 24 }}>
          {ETFS.map(e => <EntityCard key={e.id} e={e} price={price} isOpen={selected === e.id} onToggle={() => setSelected(selected === e.id ? null : e.id)} />)}
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  {["Fund","Ticker","Exchange","Sponsor Fee","Max Staking","Status"].map(h => (
                    <th key={h} style={{ padding: "10px 16px", textAlign: "left", color: "#94a3b8", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {ETFS.map(e => (
                  <tr key={e.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "10px 16px" }}><span style={{ color: "#fff", fontWeight: 700 }}>{e.sponsorFee}%</span>{e.feeWaiver && <div style={{ fontSize: 10, color: "#4ade80" }}>{e.feeWaiver}</div>}</td>
